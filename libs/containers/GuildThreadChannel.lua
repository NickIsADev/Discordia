...
function GuildThreadChannel:_loadMore(data)
	...
	if data.applied_tags then
		local applied_tags = #data.applied_tags > 0 and data.applied_tags or nil
		if self._applied_tags then
			self._applied_tags._array = applied_tags
		else
			self._applied_tags_raw = applied_tags
		end
	end
end
...
@m setTags
@t http
@p tags Forum-Tag-IDs-Resolvable	
@r boolean
@d Sets the thread's tags if it's in a forum channel.
]=]
function GuildThreadChannel:setTags(tags)
	tags = Resolver.forumTagIds(tags)
...
@r boolean
@d Applied the specified tag to the thread.
]=]
function GuildThreadChannel:applyTag(id)
	id = Resolver.forumTagId(id)
	if self:hasTag(id) then return true end
	local tags = self._applied_tags and self._applied_tags._array or self._applied_tags_raw or {}
	insert(tags, id)
	return self:_modify({applied_tags = tags or json.null})
end
...
@p id Forum-Tag-ID-Resolvable	
@r boolean
@d Removes the specified tag from the thread.
]=]
function GuildThreadChannel:removeTag(id)
	id = Resolver.forumTagId(id)
	if not self:hasTag(id) then return true end
	local tags = self._applied_tags and self._applied_tags._array or self._applied_tags_raw or {}
...
@p id Forum-Tag-ID-Resolvable
@r boolean
@d Checks whether the thread has a specific forum tag applied to it.
]=]
function GuildThreadChannel:hasTag(id)
	id = Resolver.forumTagId(id)
	local applied_tags = self._applied_tags and self._applied_tags._array or self._applied_tags_raw
	if applied_tags then
		for _, v in ipairs(applied_tags) do
			if v == id then
				return true
			end
		end
...
--[=[@p appliedTags ArrayIterable An iterable cache of all Forum Tags this tag is using. Object order is not guaranteed.]=]
function get.appliedTags(self)
	if not self._applied_tags then
		local available_tags = self._parent._available_tags
		self._applied_tags = ArrayIterable(self._applied_tags_raw, function(id)
			return available_tags:get(id)
		end)
		self._applied_tags_raw = nil
	end
